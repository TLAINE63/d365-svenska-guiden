  ...
  try {
    ...
    if ((action === "create_solution" || action === "update_solution") && req.method === "POST") {
      ...
      const payload: Record<string, unknown> = {
        ...
        sales_relevance: pick(body?.sales_relevance, RELEVANCE),
        customer_insights_relevance: pick(body?.customer_insights_relevance, RELEVANCE),
        customer_service_relevance: pick(body?.customer_service_relevance, RELEVANCE),
        field_service_relevance: pick(body?.field_service_relevance, RELEVANCE),
        contact_center_relevance: pick(body?.contact_center_relevance, RELEVANCE),
        source_status: clean(body?.source_status, 80),
        lifecycle_status: pick(body?.lifecycle_status, ["active", "end_of_sale", "legacy", "discontinued"]) || "active",
        lifecycle_note: clean(body?.lifecycle_note, 500),
        successor_solution_id: clean(body?.successor_solution_id, 100),
