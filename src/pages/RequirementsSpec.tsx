...
const RequirementsSpec = () => {
  ...
  useEffect(() => {
    trackFunnelEvent({ event_type: "analysis_start", event_name: "kravspec_erp" });
  }, []);

  useEffect(() => {
    if (step === totalSteps) {
      trackFunnelEvent({ event_type: "analysis_complete", event_name: "kravspec_erp" });
    }
  }, [step]);
  ...
  useEffect(() => {
    buyerRun.current = startBuyerToolRun("kravspecifikation", { product_key: "erp" });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await invokeWithRetry("generate-requirements", { product: "erp", industry, companySize, areas: selectedAreas });
      setResult(data);
      setStep(4);
    } catch (err: any) {
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {areaOptions
                  .filter((area) => !(isServicesIndustry(industry) && ["lager", "produktion", "transport"].includes(area.id)))
                  .map((area) => {
