...
const BcIsvCategoryPage = () => {
  const { kategori } = useParams<{ kategori: string }>();
  const page = findBcCategoryPage(kategori);
  ...
  const solutions = useMemo(() => {
    ...
    return all
      ...
      .filter((s) => !s.products?.length || s.products.includes("Business Central"))
      .sort((a, b) => a.name.localeCompare(b.name, "sv"));
  }, [all, page]);

  if (!page) return <Navigate to="/kunskapscenter/dynamics-365-tillagg/" replace />;

  const catalogHref = `/kunskapscenter/dynamics-365-tillagg/?produkt=Business%20Central&kategori=${page.categories
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        ...
        canonicalPath={`/business-central/tillagg/${page.slug}`}
        breadcrumbs={[
          { name: "Hem", url: "/" },
          { name: "Business Central", url: "/businesscentral/" },
          { name: page.h1, url: `/business-central/tillagg/${page.slug}/` },
        ]}
      />
        ...
        <div className="container mx-auto px-4 max-w-5xl">
                ...
                {solutions.map((s) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <span className="text-[11px] font-medium px-2 py-0.5 rounded border border-accent/30 bg-accent/10 text-accent">
                        {s.category}
                      </span>
                      {s.deliveryModel && (
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <p className="text-sm text-muted-foreground max-w-xl">
              Vill du jämföra fler tillägg? Hela katalogen täcker Business Central, Finance &amp;
              Supply Chain Management och Customer Engagement.
            </p>
            <Link
              to={catalogHref}
...
export default BcIsvCategoryPage;
