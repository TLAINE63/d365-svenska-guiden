          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 lg:gap-6 items-stretch">
            {buyerPaths.map((path, i) => (
              <Link
                key={path.step}
                to={path.link}
                className="group relative flex h-full min-h-[320px] flex-col items-start bg-card border border-border rounded-2xl p-6 sm:p-7 hover:border-primary/40 hover:shadow-xl transition-all duration-300"
              >
                <div className={`mb-4 flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${path.color} text-lg font-bold text-white shadow-md`}>
                  {path.step}
                </div>
                <h3 className="mb-4 min-h-[56px] w-full text-lg sm:text-xl font-bold text-foreground leading-tight">{path.title}</h3>
                <p className="mb-5 min-h-[104px] w-full text-sm text-muted-foreground leading-relaxed">{path.desc}</p>
