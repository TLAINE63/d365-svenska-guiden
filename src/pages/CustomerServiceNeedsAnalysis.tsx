...
const CustomerServiceNeedsAnalysis = () => {
 ...
 <div className={`w-10 h-10 rounded flex items-center justify-center mb-2 transition-colors ${isActive ? 'bg-[hsl(var(--hero-dark))] text-white' : isCompleted ? 'bg-[hsl(var(--hero-dark))]/20 text-[hsl(var(--hero-dark))]' : 'bg-muted text-muted-foreground'}`}>
 <Icon className="w-5 h-5" />
 </div>
 <span className={`text-xs text-center ${isActive ? 'text-[hsl(var(--hero-dark))] font-medium' : 'text-muted-foreground'}`}>{title}</span>
 </button>
 );
 })}
 </div>
