...
import { ArrowLeftRight, MessagesSquare, UserRoundPlus } from "lucide-react";
...
const PartnerDecisionActions = ({ partner, product, industry, onIntro, compact = false }: PartnerDecisionActionsProps) => {
  ...
  const compare = () => {
    const willHaveAtLeastTwo = selectedForCompare ? selected.length >= 2 : selected.length >= 1;
    if (!selectedForCompare) toggle(partner);
    trackFunnelEvent({ event_type: "cta_click", event_name: "partner_decision_action", metadata: { action: "compare", partner_slug: partner.slug, product, industry } });
    if (willHaveAtLeastTwo) navigate("/jamfor-partners/");
  };
  ...
  return (
    <div className={`grid gap-2 ${compact ? "grid-cols-1" : "sm:grid-cols-3"}`} aria-label={`Nästa steg för ${partner.name}`}>
      <Button type="button" variant="outline" onClick={compare} className="min-h-11 whitespace-normal">
        <ArrowLeftRight className="h-4 w-4" aria-hidden="true" />
        {selectedForCompare ? "Öppna jämförelse" : "Jämför"}
      </Button>
