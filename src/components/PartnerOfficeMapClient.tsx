    ...
    for (const p of partners) {
      ...
      for (const city of cities) {
        ...
        if (existing) {
          ...
          map.set(city, { city, coords: CITY_COORDS[city], partners: [partnerInfo] });
        }
      }
    }
              ...
              <Popup>
                <div className="text-sm">
                  ...
                  <ul className="space-y-0.5">
                    {partners.map(p => (
                      <li key={p.slug}>
                        <a href={`/partner/${p.slug}`} className="text-primary hover:underline">
                          {p.name}
                        </a>
                      </li>
                    ))}
...
export default PartnerOfficeMapClient;
